...
// heap-allocate a fresh &T{} per executing op (MakeDefaultGeoProc, makeCircleGeometryProcessor,
// makeEllipseGeometryProcessor, makeDIEllipseGeometryProcessor, MakeQuadPerEdgeAAProcessor/MakeTexturedQuadProcessor,
// makeFillRRectProcessor), and the op-struct recycle then dropped that GP every frame, leaving it the last un-pooled
// allocation in the createProgramInfo path — the largest in-repo allocation tier the GPU-frame benchmarks measure
...
// GP storage.
//
// Lifetime safety: a geometry processor is dead once the op that owns it (via
...
//     exactly the ownership story for the ProgramInfo/Pipeline pair.
//   - A plain full-zero recycle is complete — no capacity-preserving reset is needed. Every slice header and
...
//     structs). No GP field pins a heap object that a preserved backing would need to carry.
//
...
// recycleProgramInfo), so their GPs are simply GC'd — no aliasing, exactly as for the ProgramInfo/Pipeline pools. The
// fillRect (quad-per-edge) and fillRRect (instance attribute) geometry processors are poolable because the
// quad-per-edge GP's textured sampler slice lives in an inline array, making both full-zero-safe (see the pool var
...
var (
	...
	// The fillRect/fillRRect GPs are poolable because their shapes are full-zero-safe: the quad-per-edge GP's textured
