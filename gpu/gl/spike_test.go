...
func TestSpikeBindAttribLocationTakesEffectBeforeLink(t *testing.T) {
	...
	f.ClearColor(0, 0, 1, 1) // blue: the "nothing drew" sentinel
	f.Clear(gl.COLOR_BUFFER_BIT)
	env.checkNoError(t, "clear")

	// The mipmap program's exact position convention: a_vertex in [0,1] mapped to NDC via *2-1.
	decl := env.glslVersionDecl(t)
	vs := env.compileShader(t, gl.VERTEX_SHADER, decl+
		"in vec2 a_vertex;\nvoid main() { gl_Position = vec4(a_vertex * 2.0 - 1.0, 0.0, 1.0); }\n")
	fs := env.compileShader(t, gl.FRAGMENT_SHADER, decl+
		"out vec4 o_FragColor;\nvoid main() { o_FragColor = vec4(1.0, 0.0, 0.0, 1.0); }\n")
	program := f.CreateProgram()
	f.AttachShader(program, vs)
	f.AttachShader(program, fs)
	const loc = 3
	f.BindAttribLocationString(program, loc, "a_vertex") // must precede LinkProgram to bind on the linked program
	f.LinkProgram(program)
	var linked int32
	f.GetProgramiv(program, gl.LINK_STATUS, &linked)
	if linked == 0 {
	...
	f.GenBuffers(1, &vbo)
	f.BindBuffer(gl.ARRAY_BUFFER, vbo)
	verts := []float32{0, 0, 0, 1, 1, 0, 1, 1} // the mipmap unit quad
	f.BufferData(gl.ARRAY_BUFFER, len(verts)*4, uintptr(ptr(&verts[0])), gl.STATIC_DRAW)
	// Feed the quad on array 3 only; array 0 stays disabled so a mis-bound a_vertex (default location 0) reads nothing.
	f.EnableVertexAttribArray(loc)
	f.VertexAttribPointer(loc, 2, gl.FLOAT, false, 0, 0)
	env.checkNoError(t, "geometry upload")
	...
	got := [4]byte{pixels[center], pixels[center+1], pixels[center+2], pixels[center+3]}
	want := [4]byte{255, 0, 0, 255}
	if got != want {
		t.Fatalf("center pixel = %v, want %v (red) — the bound attribute location did not take effect, so the "+
			"quad did not rasterize", got, want)
	}
